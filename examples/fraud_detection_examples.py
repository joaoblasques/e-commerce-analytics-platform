...
def create_spark_session():
    ...
    return (
        SparkSession.builder.appName("FraudDetectionExamples")
...
def example_2_custom_fraud_rules():
    ...
    for rule_id, stats in rule_stats.items():
        ...
        print(
            f"  Avg Score When Triggered: {stats.get('avg_score_when_triggered', 0):.3f}"
        )
        print()
...
def example_3_merchant_risk_analysis():
    ...
    merchant_scores = risk_scorer.calculate_merchant_risk_scores(
        transactions_df, lookback_days=30
    )

    print("Top 10 Highest Risk Merchants:")
    high_risk_merchants = risk_scorer.get_high_risk_merchants(
        merchant_scores, min_risk_score=0.5
    )
    high_risk_merchants.select(
        "merchant_id",
        "merchant_risk_score",
        "merchant_risk_level",
        "transaction_count",
    ...
    detailed_analysis = risk_scorer.analyze_merchant_transaction_patterns(
        transactions_df, sample_merchant
    )
    ...
    risk_scorer.save_risk_profiles(
        merchant_scores, "data/output/merchant_risk_profiles"
    )
...
def example_4_alert_prioritization():
    ...
    if fraud_alerts.count() > 0:
        ...
        # Get priority queue
        priority_queue = alert_prioritizer.get_priority_queue(
            prioritized_alerts, max_alerts=20
        )

        print("Top Priority Fraud Alerts:")
        priority_queue.select(
            "transaction_id",
            "customer_id",
            "price",
            "alert_priority",
        ...
        summary_metrics = alert_prioritizer.get_alert_summary_metrics(
            prioritized_alerts
        )
        ...
        print(
            f"Total Financial Exposure: ${summary_metrics['overall_stats']['total_financial_exposure']:,.2f}"
        )
        print(
            f"Estimated Investigation Time: {summary_metrics['overall_stats']['total_investigation_time_hours']:.1f} hours"
        )

        print("\nPriority Breakdown:")
        for priority, stats in summary_metrics["priority_breakdown"].items():
            print(
                f"  {priority}: {stats['count']} alerts (${stats['total_amount']:,.2f})"
            )

        # Save prioritized alerts
        alert_prioritizer.save_prioritized_alerts(
            prioritized_alerts, "data/output/prioritized_alerts"
        )
    ...
    try:
        ...
        if "error" not in metrics:
            ...
            print(
                ...
                metrics.get("message", metrics["error"]),
            )
