...
class StreamingTransformationsDemo:
    ...
    def _setup_spark(self):
        ...
        try:
            self.spark = SparkSession.builder \
                .appName("StreamingTransformationsDemo") \
                .master("local[*]") \
    ...
    def demonstrate_data_enrichment(self):
        ...
        try:
            ...
            sample_data = [
                ("user_001", "tx_001", "prod_123", datetime.now(), 99.99, "credit_card", "New York", "Electronics/Phones", 99.99, 1, "Gold", 1500.0),
                ("user_002", "tx_002", "prod_456", datetime.now(), 249.50, "paypal", "California", "Clothing/Shoes", 249.50, 1, "Silver", 800.0),
                ("user_003", "tx_003", "prod_789", datetime.now(), 1299.99, "credit_card", "Texas", "Electronics/Computers", 1299.99, 1, "Premium", 3000.0)
            ]
            ...
            enriched_df.select(
                "user_id", "total_amount", "customer_tier_normalized", 
                "ltv_category", "price_category", "time_period",
                "risk_category", "transaction_size"
            ).show(truncate=False)
    ...
    def demonstrate_real_time_aggregations(self):
        ...
        try:
            ...
            kpi_df.select(
                "window", "region", "total_revenue", "total_orders",
                "unique_customers", "avg_order_value", "revenue_per_customer"
            ).show(truncate=False)
    ...
    def demonstrate_stream_joins(self):
        ...
        try:
            ...
            tx_data = [
                ("user_001", base_time + timedelta(minutes=5), "tx_001", "prod_123", 99.99, "credit_card", "New York"),
                ("user_002", base_time + timedelta(minutes=8), "tx_002", "prod_456", 149.50, "paypal", "California")
            ]
            
            # Sample behavior data (events leading to transactions)
            behavior_data = [
                ("user_001", base_time + timedelta(minutes=2), "sess_001", "product_view", "prod_123", 60, 70),
                ("user_001", base_time + timedelta(minutes=3), "sess_001", "add_to_cart", "prod_123", 80, 85),
                ("user_002", base_time + timedelta(minutes=6), "sess_002", "search", None, 40, 30),
                ("user_002", base_time + timedelta(minutes=7), "sess_002", "product_view", "prod_456", 65, 75)
            ]
            ...
            correlated_df.select(
                "tx_user_id", "tx_amount", "behavior_event_type",
                "time_to_purchase_minutes", "product_match", "conversion_path"
            ).show(truncate=False)
    ...
    def demonstrate_deduplication(self):
        ...
        try:
            ...
            sample_data = [
                ...
                ("user_001", "tx_001", base_time + timedelta(seconds=5), "prod_123", 99.99, "credit_card"),  # Exact duplicate
                ("user_001", "tx_002", base_time + timedelta(minutes=1), "prod_123", 99.95, "credit_card"),  # Similar amount
                ("user_002", "tx_003", base_time + timedelta(minutes=2), "prod_456", 149.50, "paypal"),
                ("user_002", "tx_004", base_time + timedelta(minutes=3), "prod_789", 75.00, "cash"),
                ("user_001", "tx_005", base_time + timedelta(minutes=4), "prod_123", 99.99, "credit_card")   # Potential duplicate
            ]
            ...
            exact_dedup_df.select(
                ...
                "duplicate_type", "deduplication_method"
            ).show(truncate=False)
            ...
            tx_dedup_df.select(
                "user_id", "transaction_id", "total_amount",
                "duplicate_type", "overall_similarity"
            ).show(truncate=False)
