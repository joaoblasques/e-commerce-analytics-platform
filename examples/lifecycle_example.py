...
def main():
    ...
    spark = (
        SparkSession.builder.appName("LifecycleExample")
        .master("local[2]")
    ...
    try:
        ...
        delta_manager = DeltaLakeManager(
            base_path="file:///tmp/delta-lake-example", spark_session=spark
        )
        ...
        print(
            f"✅ Loaded retention rule: {transactions_rule.hot_days}/{transactions_rule.warm_days}/{transactions_rule.cold_days} days"
        )
        print(
            f"✅ Loaded archive policy: {archive_policy.format} format, {archive_policy.compression} compression"
        )
        ...
        processed_df = (
            sample_df.select("user_id", "amount", "transaction_date")
        ...
        if storage_metrics:
            ...
            for table_name, metrics in storage_metrics.items():
                ...
                print(
                    f"   📋 {table_name}: {size_gb:.2f} GB, ${metrics.storage_cost_usd:.2f}, {metrics.tier.value} tier"
                )
        ...
        print(
            f"   Monthly cost: ${report['storage_summary']['total_monthly_cost_usd']:.2f}"
        )
        ...
        report_file = (
            f"lifecycle_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        if config_summary["retention"]["archive_after_days"]:
            print(
                f"   Archive after: {config_summary['retention']['archive_after_days']} days"
            )
        if config_summary["retention"]["delete_after_days"]:
            print(
                f"   Delete after: {config_summary['retention']['delete_after_days']} days"
            )
