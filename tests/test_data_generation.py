...
class TestDataGenerationConfig:
    ...
    def test_hourly_multiplier(self):
        ...
        assert config.get_hourly_multiplier(10) == 3.0  # Peak hour
        assert config.get_hourly_multiplier(14) == 3.0  # Peak hour (2 PM)
        assert config.get_hourly_multiplier(20) == 3.0  # Peak hour

        # Regular business hours (non-peak)
...
class TestGeographicPatterns:
    ...
    def test_distance_calculation(self):
        ...
        ny = GeographicLocation(
            40.7128, -74.0060, "US", "NY", "New York", "UTC-5", "North America"
        )
        la = GeographicLocation(
            34.0522, -118.2437, "US", "CA", "Los Angeles", "UTC-8", "North America"
        )
    ...
    def test_location_anomaly_detection(self):
        ...
        # Same location
        ny1 = GeographicLocation(
            40.7128, -74.0060, "US", "NY", "New York", "UTC-5", "North America"
        )
        ny2 = GeographicLocation(
            40.7130, -74.0062, "US", "NY", "New York", "UTC-5", "North America"
        )

        assert not patterns.is_location_anomaly(ny1, ny2, threshold_km=500)

        # Different continents
        tokyo = GeographicLocation(
            35.6762, 139.6503, "JP", "Tokyo", "Tokyo", "UTC+9", "Asia"
        )
...
class TestFraudPatterns:
    ...
    def test_velocity_attack_generation(self):
        ...
        assert all(
            attack["fraud_type"] == FraudType.VELOCITY_ATTACK.value
...
class TestSeasonalPatterns:
    ...
    def test_seasonal_boost_application(self):
        ...
        boosted = patterns.apply_seasonal_boost(
            "Electronics", base_prob, december_timestamp
        )
        assert boosted > base_prob

        # Random category should get less boost
        other_boosted = patterns.apply_seasonal_boost(
            "Pet Supplies", base_prob, december_timestamp
        )
