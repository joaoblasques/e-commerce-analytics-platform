...
def test_start_command_default(runner):
    ...
    assert result.exit_code == 0
    assert (
        "Starting E-Commerce Analytics Platform with config: config/local.yaml"
        in result.output
    )
...
def test_start_command_custom_config(runner):
    ...
    assert result.exit_code == 0
    assert (
        "Starting E-Commerce Analytics Platform with config: custom.yaml"
        in result.output
    )
...
def test_start_command_short_option(runner):
    ...
    assert result.exit_code == 0
    assert (
        "Starting E-Commerce Analytics Platform with config: short.yaml"
        in result.output
    )
