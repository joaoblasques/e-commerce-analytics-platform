...
from manage_kafka import TOPIC_CONFIGS, KafkaManager
...
class TestKafkaSetup:
    ...
    def test_topic_configs_structure(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            assert isinstance(
                topic_name, str
            ), f"Topic name {topic_name} should be string"
            ...
            assert isinstance(
                config["partitions"], int
            ), f"Partitions should be integer for {topic_name}"
            assert (
                config["partitions"] > 0
            ), f"Partitions should be positive for {topic_name}"

            # Validate replication factor
            assert isinstance(
                config["replication_factor"], int
            ), f"Replication factor should be integer for {topic_name}"
            assert (
                config["replication_factor"] > 0
            ), f"Replication factor should be positive for {topic_name}"

            # Validate configs
            assert isinstance(
                config["configs"], dict
            ), f"Configs should be dict for {topic_name}"
            assert (
                len(config["configs"]) > 0
            ), f"Configs should not be empty for {topic_name}"

            # Validate description
            assert isinstance(
                config["description"], str
            ), f"Description should be string for {topic_name}"
            assert (
                len(config["description"]) > 0
            ), f"Description should not be empty for {topic_name}"
    ...
    def test_topic_retention_policies(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            ...
            assert (
                segment_ms <= retention_ms
            ), f"Segment should not exceed retention for {topic_name}"
    ...
    def test_topic_compression_settings(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            ...
            assert (
                "compression.type" in configs
            ), f"Topic {topic_name} missing compression.type"

            compression_type = configs["compression.type"]
            assert (
                compression_type in valid_compression_types
    ...
    def test_topic_cleanup_policies(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            ...
            assert (
                "cleanup.policy" in configs
            ), f"Topic {topic_name} missing cleanup.policy"

            cleanup_policy = configs["cleanup.policy"]
            assert (
                cleanup_policy in valid_cleanup_policies
    ...
    def test_topic_message_size_limits(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            ...
            assert (
                "max.message.bytes" in configs
            ), f"Topic {topic_name} missing max.message.bytes"

            max_message_bytes = int(configs["max.message.bytes"])
            assert (
                max_message_bytes > 0
            ), f"Max message bytes should be positive for {topic_name}"
            assert (
    ...
    def test_topic_naming_conventions(self):
        ...
        for topic_name in TOPIC_CONFIGS.keys():
            ...
            assert (
                " " not in topic_name
            ), f"Topic name {topic_name} should not contain spaces"
            assert (
                topic_name.replace("-", "").replace("_", "").isalnum()
    ...
    def test_partition_distribution_strategy(self):
        ...
        total_partitions = sum(
            config["partitions"] for config in TOPIC_CONFIGS.values()
        )
        assert (
            total_partitions <= 50
        ), f"Total partitions too high for development: {total_partitions}"

        # No single topic should dominate
        max_partitions = max(config["partitions"] for config in TOPIC_CONFIGS.values())
        assert (
            max_partitions <= 15
        ), f"Single topic has too many partitions: {max_partitions}"
    ...
    def test_min_insync_replicas_configuration(self):
        ...
        for topic_name, config in TOPIC_CONFIGS.items():
            ...
            assert (
                "min.insync.replicas" in configs
...
class TestKafkaScriptFunctionality:
    ...
    def test_script_file_exists(self):
        ...
        script_path = os.path.join(
            os.path.dirname(__file__), "..", "scripts", "manage_kafka.py"
        )
    ...
    def test_shell_script_exists(self):
        ...
        script_path = os.path.join(
            os.path.dirname(__file__), "..", "scripts", "kafka-topics.sh"
        )
    ...
    def test_documentation_exists(self):
        ...
        doc_path = os.path.join(
            os.path.dirname(__file__), "..", "docs", "kafka-setup.md"
        )
            ...
            if manager.connect():
                ...
                if result:
                    ...
                    assert (
                        ...
                        == TOPIC_CONFIGS["transactions"]["partitions"]
                    )
