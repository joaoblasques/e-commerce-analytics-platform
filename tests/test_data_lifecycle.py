...
from src.data_lake.lifecycle_config import (
...
class TestDataLifecycleManager:
    ...
    def spark(self):
        ...
        return (
            SparkSession.builder.appName("test_lifecycle")
            .master("local[2]")
    ...
    def test_initialization(self, mock_delta_manager):
        ...
        with patch.object(
            DataLifecycleManager, "_initialize_metadata_tables"
        ) as mock_init:
            manager = DataLifecycleManager(mock_delta_manager)
    ...
    def test_apply_retention_policies_dry_run(
        self, lifecycle_manager, sample_retention_rule, spark
    ):
    ...
    def test_get_lineage_graph(self, lifecycle_manager, sample_lineage_record, spark):
        ...
        lineage_data = [
            (
                "test_table",
                "CREATE",
                sample_lineage_record.timestamp,
                '["source1", "source2"]',
                "test_table",
                1000,
                1048576,
    ...
    def test_optimize_storage_costs(self, lifecycle_manager, spark):
        ...
        # Mock table list
        lifecycle_manager.delta_manager.tables = {
            "test_table1": "path1",
            "test_table2": "path2",
        }
    ...
    def test_generate_lifecycle_report(self, lifecycle_manager, sample_retention_rule):
        ...
        # Mock methods
        lifecycle_manager.apply_retention_policies = Mock(
            return_value={
                "test_table": {
                    "hot_records": 1000,
                    "warm_records": 500,
        ...
        lifecycle_manager.optimize_storage_costs = Mock(
            return_value={
    ...
    def test_configuration_save_load(
        self, lifecycle_manager, sample_retention_rule, sample_archive_policy
    ):
...
class TestLifecycleConfigManager:
    ...
    def test_validate_configuration(self, config_manager):
        ...
        assert len(issues) > 0
        assert any(
            "hot_days should be less than warm_days" in issue for issue in issues
        )
        assert any(
            "warm_days should be less than cold_days" in issue for issue in issues
        )
...
class TestIntegration:
    ...
    def spark(self):
        ...
        return (
            SparkSession.builder.appName("test_integration")
            .master("local[2]")
            .config("spark.sql.shuffle.partitions", "2")
            .getOrCreate()
        )
    ...
    def test_end_to_end_lifecycle_management(self, spark):
        ...
        assert (
            retention_results["test_table"]["hot_records"] == 100
        )  # 5 and 15 days ago
