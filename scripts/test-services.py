...
def test_spark() -> Dict[str, Any]:
    ...
    # Overall status
    healthy_components = sum(1 for comp in spark_status.values() if comp['status'] == 'healthy')
...
def print_results(results: Dict[str, Dict[str, Any]]) -> None:
    ...
    for service_name, result in results.items():
        ...
        status_icon = "✅" if status == 'healthy' else "⚠️" if status == 'partial' else "❌"
...
def main():
    ...
    for service_name, test_func in tests:
        ...
        try:
            ...
            print(f"{'✅' if status == 'healthy' else '⚠️' if status == 'partial' else '❌'}")
        except Exception as e:
