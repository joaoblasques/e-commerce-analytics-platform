...
class KafkaManager:
        ...
        try:
            ...
            for topic_name in topics_to_create:
                if topic_name in existing_topics:
                    config = TOPIC_CONFIGS[topic_name]
                    logger.info(f"✅ {topic_name}: {config['partitions']} partitions, {config['description']}")
                else:
        ...
        try:
            ...
            if topic_name in metadata:
                ...
                result = {
                    ...
                    'configs': {k: v.value for k, v in topic_config.items()} if hasattr(topic_config, 'items') else {},
                    'partition_details': []
                }
    ...
    def produce_test_message(self, topic_name: str, message: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Test message sent to {topic_name} "
    ...
    if success:
        ...
        for topic_name in topics_to_show:
            if topic_name in TOPIC_CONFIGS:
                ...
                click.echo(f"   Retention: {int(config['configs']['retention.ms']) // (24*60*60*1000)} days")
...
def describe_topic(topic_name):
    ...
    if topic_info:
        ...
        for partition in topic_info['partition_details']:
            click.echo(f"  Partition {partition['partition_id']}: "
...
@cli.command()
@click.argument('topic_name')
@click.option('--message', '-m', default='{"test": "message", "timestamp": "2024-01-01T00:00:00Z"}')
