...
@click.option(
    "--count", "-n", type=int, default=1000, help="Number of records to generate"
)
@click.option(
...
def generate(ctx, data_type, count, output, start_date, end_date, pretty):
    ...
    if data_type == "transactions" or data_type == "all":
        ...
        transactions = generator.generate_transactions(
            count if data_type == "transactions" else count // 3, start_time, end_time
        )
    ...
    if data_type == "user-events" or data_type == "all":
        ...
        events = generator.generate_user_events(
            count if data_type == "user-events" else count // 3, start_time, end_time
        )
        data.extend(events)
...
@click.option(
    ...
    "-s",
    type=click.Choice(
        ["normal_traffic", "peak_traffic", "fraud_testing", "maintenance_mode"]
    ),
    default="normal_traffic",
    help="Streaming scenario",
)
@click.option("--duration", "-d", type=float, default=1.0, help="Duration in hours")
@click.option(
    "--topics",
    "-t",
    multiple=True,
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be done without actually streaming"
)
@click.pass_context
def stream(ctx, scenario, duration, topics, rate_multiplier, dry_run):
    ...
    try:
        if topics:
            ...
            click.echo(
                f"  Messages per second: {stats.get('messages_per_second', 0):.2f}"
            )
...
@click.option(
    "--batch-size", "-b", type=int, default=1000, help="Batch size for sending data"
)
@click.pass_context
...
@cli.command()
@click.option(
    "--kafka-servers", "-k", default="localhost:9092", help="Kafka bootstrap servers"
)
@click.pass_context
def health(ctx, kafka_servers):
    ...
    try:
        ...
        click.echo(
            f"  📊 Users: {stats['total_users']}, Products: {stats['total_products']}"
        )
    except Exception as e:
    ...
    try:
        producer = KafkaDataProducer(
            generator, bootstrap_servers=kafka_servers.split(",")
        )
        ...
        if health_status["overall_status"] == "HEALTHY":
            ...
            click.echo(
                f"  📋 Available topics: {len(health_status.get('available_topics', []))}"
            )
        else:
