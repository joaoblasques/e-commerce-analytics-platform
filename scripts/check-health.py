    ...
    def check_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "swap_memory": {
                    "status": "healthy" if swap.percent < 50 else "warning" if swap.percent < 80 else "critical",
            ...
            if isinstance(result, dict):
                if "status" in result:
                    ...
                    if status in ["unhealthy", "critical", "error"]:
                        ...
                        critical_issues.append(f"{category}: {result.get('error', 'Unknown issue')}")
                    elif status == "warning":
                    ...
                    for service, service_result in result.items():
                        if isinstance(service_result, dict) and "status" in service_result:
                            ...
                            if status in ["unhealthy", "critical", "error"]:
                                overall_status = "UNHEALTHY"
                                critical_issues.append(f"{category}.{service}: {service_result.get('error', 'Unknown issue')}")
                            elif status == "warning":
                                warnings.append(f"{category}.{service}: Warning condition detected")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Comprehensive health check for e-commerce analytics platform')
    parser.add_argument('--no-monitoring', action='store_true', help='Skip monitoring services check')
    parser.add_argument('--no-docker', action='store_true', help='Skip Docker status check')
    ...
    for category, result in results.items():
        ...
        if isinstance(result, dict):
            if "status" in result and result["status"] in ["unhealthy", "critical", "error"]:
                overall_healthy = False
                break
            else:
                for service_result in result.values():
                    if isinstance(service_result, dict) and "status" in service_result:
                        if service_result["status"] in ["unhealthy", "critical", "error"]:
                            overall_healthy = False
                            break
