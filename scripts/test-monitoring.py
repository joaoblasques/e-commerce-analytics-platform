...
def test_prometheus_targets():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(
                f"Prometheus targets: {healthy_count}/{total_count} healthy",
            ...
            print_status(
                f"Failed to get Prometheus targets: HTTP {response.status_code}",
                "ERROR",
            )
            return False
...
def test_grafana_datasources():
    ...
    try:
        ...
        response = requests.get(
            "http://localhost:3000/api/datasources", auth=auth, timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            print_status(
                f"Failed to get Grafana datasources: HTTP {response.status_code}",
                "ERROR",
            )
            return False
...
def main():
    ...
    for result in results:
        ...
        if status == "healthy":
            ...
            print_status(
                f"{name:20} HEALTHY (response time: {response_time:.3f}s)", "SUCCESS"
            )
        else:
    ...
    print_status(
        f"Monitoring Services: {healthy_services}/{total_services} healthy",
    ...
    # Overall status
    overall_success = (
        healthy_services == total_services and prometheus_ok and grafana_ok
    )
