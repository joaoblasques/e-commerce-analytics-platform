...
class TestDataGenerator:
    ...
    def generate_orders(
        self, customer_count: int, product_count: int, count: int = 2000
    ) -> List[Dict[str, Any]]:
        """Generate order data."""
    ...
    def generate_order_items(
        self, order_count: int, product_count: int
    ) -> List[Dict[str, Any]]:
    ...
    def generate_all_data(
        self, customers: int = 1000, products: int = 500, orders: int = 2000
    ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for e-commerce analytics platform"
    )
    parser.add_argument(
        "--customers",
        type=int,
