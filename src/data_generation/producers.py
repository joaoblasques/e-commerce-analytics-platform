...
class KafkaDataProducer:
    ...
    def _stream_to_topic(
        self, topic: str, rate_multiplier: float, duration_hours: float
    ) -> None:
        ...
        end_time = (
            start_time + timedelta(hours=duration_hours) if duration_hours else None
        )
    ...
    def _calculate_current_rate(self, base_rate: float, burst_factor: float) -> float:
        ...
        temporal_context = self.generator.temporal_patterns.get_temporal_context(
            current_time
        )
    ...
    def _send_to_kafka(
        self, topic: str, data: Dict[str, Any], key_extractor: Callable
    ) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Sent to {topic} (partition: {record_metadata.partition}, offset: {record_metadata.offset})"
            )
    ...
    def send_batch(
        self, topic: str, data: List[Dict[str, Any]], key_extractor: Callable = None
    ) -> int:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["start_time"]:
            ...
            stats["messages_per_second"] = (
                stats["messages_sent"] / elapsed if elapsed > 0 else 0
            )
            stats["bytes_per_second"] = (
                stats["bytes_sent"] / elapsed if elapsed > 0 else 0
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_topics = [
                topic
                for topic in self.topic_config.keys()
...
class DataGenerationOrchestrator:
    ...
    def __init__(
        ...
        use_reliable_producer: bool = True,
        ...
        # Choose producer type based on configuration
        if use_reliable_producer:
            self.producer = ReliableKafkaProducer(
                redis_client=redis_client, enable_deduplication=True
            )
            # Setup default alert handler
            self.producer.add_health_alert_callback(default_alert_handler)
        else:
            # Legacy producer for backward compatibility
    ...
    def generate_historical_data(self, days: int = 30) -> Dict[str, int]:
        ...
        events = self.generator.generate_user_events(total_events, start_time, end_time)
        ...
        results["product-updates"] = self.producer.send_batch(
