...
class ECommerceDataGenerator:
    ...
    def _generate_users(self) -> None:
        ...
        for i in range(self.config.total_users):
            ...
            browser = np.random.choice(
                list(self.config.browsers.keys()), p=list(self.config.browsers.values())
            )
    ...
    def generate_transactions(
        self, count: int, start_time: datetime = None, end_time: datetime = None
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            active_users = [
                uid for uid, user in self.users.items() if user["is_active"]
            ]
            ...
            if is_fraud:
                ...
                fraud_indicators = self._apply_fraud_pattern(
                    user, temporal_context, fraud_type
                )
    ...
    def generate_product_updates(self, count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            update_type = random.choice(
                ["price_change", "stock_update", "description_update", "status_change"]
            )
    ...
    def _generate_weighted_timestamp(
    ...
    def _select_products_for_user(
        ...
        for _ in range(num_products):
            ...
            selected_category = np.random.choice(
                self.config.categories, p=category_probs
            )
    ...
    def _apply_fraud_pattern(
        self, user: Dict[str, Any], temporal_context, fraud_type: FraudType
    ) -> Dict[str, Any]:
        ...
        if fraud_type == FraudType.LOCATION_ANOMALY:
            ...
            fraud_location = self.fraud_patterns.generate_location_anomaly(
                user_location
            )
            fraud_indicators["anomaly_location"] = fraud_location.__dict__
            fraud_indicators[
                "distance_km"
            ] = self.geographic_patterns.calculate_distance(
            ...
            fraud_indicators[
                "anomaly_amount"
            ] = self.fraud_patterns.generate_amount_anomaly(avg_amount)
            ...
            fraud_indicators[
                "anomaly_time"
            ] = self.fraud_patterns.generate_time_anomaly(user["location"]["timezone"])

        elif fraud_type == FraudType.DEVICE_ANOMALY:
            # Generate new/unusual device
            fraud_indicators[
                "anomaly_device"
            ] = self.fraud_patterns.generate_device_anomaly()
            ...
            fraud_indicators[
                "anomaly_category"
            ] = self.fraud_patterns.generate_pattern_anomaly(user_categories)
    ...
    def _create_user_event(
        ...
        if event_type in ["page_view", "product_view"]:
            ...
            event_data["page_url"] = self._generate_page_url(
                event_type, event_data.get("category")
            )
    ...
    def _generate_page_url(self, event_type: str, category: str = None) -> str:
        ...
        base_urls = {
            ...
            "product_view": [
                f'/products/{category.lower().replace(" ", "-")}'
                if category
                else "/products"
            ],
        }
    ...
    def _generate_search_query(self) -> str:
        ...
        return template.format(
            category=category.lower(), adjective=adjective, brand=brand
        )

    def _get_product_field_value(
        self, product: Dict[str, Any], update_type: str
    ) -> Any:
    ...
    def _generate_new_product_value(
        self, product: Dict[str, Any], update_type: str
    ) -> Any:
        ...
        if update_type == "price_change":
            ...
            return self._generate_product_description(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_products": len(
                [p for p in self.products.values() if p["is_active"]]
            ),
