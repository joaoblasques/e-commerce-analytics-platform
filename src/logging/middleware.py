...
class LoggingMiddleware(BaseHTTPMiddleware):
        ...
        if self.log_request_body and request.headers.get("content-length"):
            ...
            if content_length <= self.max_body_size:
                try:
                    ...
                    if body:
                        log_data["request_body"] = body.decode("utf-8", errors="ignore")[:self.max_body_size]
                except Exception:
    ...
    async def _log_response(self, request: Request, response: Response, 
                           duration_ms: float, ctx: CorrelationContext):
        ...
        if (self.log_response_body and 
            (response.status_code >= 400 or 
             (response.headers.get("content-length") and 
              int(response.headers["content-length"]) <= self.max_body_size))):
        ...
        try:
            ...
            if duration_ms > self.slow_threshold:
                ...
                getattr(self.logger, severity)(
                    ...
                    duration_ms=duration_ms,
                    threshold_exceeded=self.very_slow_threshold if duration_ms > self.very_slow_threshold else self.slow_threshold,
                    http_method=request.method,
                    http_path=str(request.url.path),
