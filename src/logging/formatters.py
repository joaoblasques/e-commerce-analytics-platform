...
class JSONFormatter(logging.Formatter):
    ...
    def __init__(self, service_name: str = "ecap-service", 
...
class ECSFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, 'http'):
            ...
            if "duration_ms" in http_data:
                ecs_entry["event"] = {
                    "duration": http_data["duration_ms"] * 1000000  # Convert to nanoseconds
                }
            ...
            for key, value in extra_fields.items():
                if key.startswith('user_'):
                    if 'user' not in ecs_entry:
                        ecs_entry['user'] = {}
                    ecs_entry['user'][key[5:]] = value  # Remove 'user_' prefix
                elif key.startswith('kafka_'):
                    if 'kafka' not in ecs_entry:
                        ecs_entry['kafka'] = {}
                    ecs_entry['kafka'][key[6:]] = value  # Remove 'kafka_' prefix
                else:
                    # Add as custom field
                    if 'custom' not in ecs_entry:
                        ecs_entry['custom'] = {}
                    ecs_entry['custom'][key] = value
    ...
    def _extract_extra_fields(self, record: logging.LogRecord) -> Dict[str, Any]:
        ...
        standard_fields = {
            ...
            'lineno', 'funcName', 'created', 'msecs', 'relativeCreated',
            'thread', 'threadName', 'processName', 'process', 'message',
            'correlation_id', 'trace_id', 'span_id', 'http', 'database'
        }
...
class CompactJSONFormatter(JSONFormatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, 'http'):
            http = record.http
            log_entry["http"] = f"{http.get('method')} {http.get('path')} {http.get('status_code')}"
