...
class StreamingAggregator:
    ...
    def create_real_time_kpis(
        ...
        try:
            # Create time windows
            windowed_df = (
                transaction_df.withWatermark("timestamp", "2 minutes")
                .groupBy(
                ...
                .agg(
                    ...
                    # Quality metrics
                    spark_sum(
                        when(col("risk_category") == "High", 1).otherwise(0)
                    ).alias("high_risk_orders"),
                    spark_sum(
                        when(col("payment_method") == "credit_card", 1).otherwise(0)
            ...
            kpi_df = (
                ...
                .withColumn(
                    "items_per_order", col("total_items_sold") / col("total_orders")
                )
                .withColumn(
    ...
    def create_customer_behavior_aggregations(
        ...
        try:
            ...
            windowed_df = (
                behavior_df.withWatermark("timestamp", "3 minutes")
                .groupBy(
                    window(col("timestamp"), window_duration, slide_duration),
                    col("user_id"),
                )
                .agg(
                    ...
                    spark_sum(
                        when(col("event_type") == "product_view", 1).otherwise(0)
                    ).alias("product_views"),
                    spark_sum(
            ...
            behavior_metrics_df = (
                ...
                .withColumn(
                    "session_efficiency",
                    col("total_engagement_score")
                    / col("total_session_time")
                    * 60,  # per minute
                )
                .withColumn(
                    "conversion_rate",
            ...
            self.logger.info(
                f"Created customer behavior aggregations with {window_duration} windows"
            )
    ...
    def create_product_performance_aggregations(
        ...
        try:
            ...
            product_metrics_df = (
                windowed_df.withColumn(
                    "revenue_per_unit", col("revenue") / col("units_sold")
                )
                .withColumn(
                    "buyer_penetration",
            ...
            self.logger.info(
                f"Created product performance aggregations with {window_duration} windows"
            )
    ...
    def create_session_aggregations(
        self, behavior_df: "DataFrame", session_timeout: str = "30 minutes"
    ) -> "DataFrame":
        """
        ...
        try:
            ...
            session_metrics_df = (
                ...
                .withColumn(
                    "session_duration_minutes", col("session_duration_seconds") / 60.0
                )
                .withColumn(
                    "engagement_per_minute",
                    when(
                        col("session_duration_minutes") > 0,
                        col("session_engagement_score")
                ...
                .withColumn(
                    "funnel_progression",
                    when(
                        (col("entry_funnel_stage") == "Awareness")
                        & (col("exit_funnel_stage") == "Purchase"),
                        lit("Full Conversion"),
                    )
                    .when(
                        col("exit_funnel_stage") != col("entry_funnel_stage"),
                        lit("Progressed"),
                    )
            ...
            self.logger.info(
                f"Created session aggregations with {session_timeout} timeout"
            )
    ...
    def create_alert_aggregations(
        ...
        try:
            ...
            tx_alerts = (
                transaction_df.withWatermark("timestamp", "1 minute")
                ...
                .agg(
                    ...
                    spark_sum(when(col("total_amount") > 1000, 1).otherwise(0)).alias(
                        "large_transaction_count"
                    ),
                    # Quality alerts
                    spark_sum(
                        when(col("anomaly_flags") != "none", 1).otherwise(0)
                    ).alias("anomaly_count"),
            ...
            behavior_alerts = (
                behavior_df.withWatermark("timestamp", "1 minute")
                .groupBy(window(col("timestamp"), alert_window))
                .agg(
                    # Activity alerts
                    ...
                    spark_sum(
                        when(col("engagement_level") == "High", 1).otherwise(0)
                    ).alias("high_engagement_events"),
                    current_timestamp().alias("alert_timestamp"),
                )
            )
            ...
            combined_alerts = tx_alerts.join(
                ...
                coalesce(tx_alerts.high_risk_count, lit(0)).alias(
                    "high_risk_transactions"
                ),
                coalesce(tx_alerts.anomaly_count, lit(0)).alias("anomalies_detected"),
                coalesce(behavior_alerts.behavior_volume, lit(0)).alias(
                    "behavior_events"
                ),
                coalesce(behavior_alerts.active_sessions, lit(0)).alias(
                    "active_sessions"
                ),
                coalesce(behavior_alerts.avg_engagement, lit(0.0)).alias(
            ...
            alert_df = (
                ...
                .withColumn(
                    "risk_alert",
                    when(
                        col("high_risk_transactions") > 5, lit("HIGH_RISK_DETECTED")
                    ).otherwise(lit("NORMAL")),
                )
                .withColumn(
                    "anomaly_alert",
                    when(
                        col("anomalies_detected") > 0, lit("ANOMALIES_DETECTED")
                    ).otherwise(lit("NORMAL")),
                )
                .withColumn(
                    "engagement_alert",
