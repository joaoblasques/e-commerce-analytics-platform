...
class StreamJoinEngine:
    ...
    def join_transaction_behavior_streams(
        ...
        try:
            ...
            behavior_watermarked = behavior_df.withWatermark(
            ...
            correlated_df = (
                joined_df.withColumn(
                    ...
                    (
                        col("tx_timestamp").cast("long")
                        - col("behavior_timestamp").cast("long")
                    )
                    / 60,
                )
                .withColumn(
                    "product_match",
                    when(
                        col("tx_product_id") == col("behavior_product_id"), lit(True)
                    ).otherwise(lit(False)),
                )
                .withColumn(
                ...
                .withColumn(
                    "conversion_path",
                    when(
                        col("behavior_event_type") == "add_to_cart", lit("Direct Cart")
                    )
                    .when(
                        col("behavior_event_type") == "product_view",
                        lit("Product Browse"),
                    )
                ...
                .withColumn("correlation_timestamp", current_timestamp())
            )

            self.logger.info(
                f"Successfully joined transaction and behavior streams with {join_window} window"
            )
    ...
    def create_user_journey_stream(
        ...
        try:
            ...
            behavior_with_sequence = (
                behavior_df.withWatermark("timestamp", "5 minutes")
                .withColumn(
                    "event_sequence",
                    row_number().over(
            ...
            transaction_events = transaction_df.withWatermark(
                "timestamp", "5 minutes"
            ).select(
                col("user_id"),
            ...
            journey_df = behavior_with_sequence.join(
                ...
                & (
                    ...
                    >= behavior_with_sequence.event_timestamp
                )
                & (
                    transaction_events.purchase_timestamp
                    <= behavior_with_sequence.event_timestamp
                    + expr(f"interval '{journey_window}'")
                ),
                "left_outer",
            )
            ...
            journey_analyzed = (
                journey_df.withColumn(
                    "led_to_purchase",
                    when(col("transaction_id").isNotNull(), lit(True)).otherwise(
                        lit(False)
                    ),
                )
                .withColumn(
                    ...
                    when(
                        ...
                        (
                            col("purchase_timestamp").cast("long")
                ...
                .withColumn(
                    "product_correlation",
                    when(
                        col("product_id") == col("purchased_product_id"), lit("Direct")
                    )
                    .when(
                        col("product_id").isNotNull()
                        & col("purchased_product_id").isNotNull(),
                        lit("Related"),
                    )
                ...
                .withColumn("journey_timestamp", current_timestamp())
            )

            self.logger.info(
                f"Created user journey stream with {journey_window} window"
            )
    ...
    def join_with_customer_profile_stream(
        ...
        try:
            ...
            profile_versioned = (
                customer_profile_updates_df.withWatermark("timestamp", "10 minutes")
                .withColumn(
                    "profile_version",
                    row_number().over(
                        Window.partitionBy("user_id").orderBy(col("timestamp").desc())
                    ),
                )
                .select(
            ...
            enriched_df = transaction_df.withWatermark("timestamp", "5 minutes").join(
            ...
            profile_enriched = (
                ...
                .withColumn(
                    ...
                    when(
                        (col("current_customer_tier") == "Bronze")
                        & (col("current_ltv") > 500),
                        lit(True),
                    )
                    .when(
                        (col("current_customer_tier") == "Silver")
                        & (col("current_ltv") > 1000),
                        lit(True),
                    )
                    .when(
                        (col("current_customer_tier") == "Gold")
                        & (col("current_ltv") > 2000),
                        lit(True),
                    )
                    .otherwise(lit(False)),
                )
                ...
                .withColumn(
                    "profile_freshness",
                    when(
                        col("profile_update_timestamp").isNotNull(),
                        (
            ...
            self.logger.info(
                f"Joined transactions with customer profiles using {profile_window} window"
            )
    ...
    def create_cross_stream_correlation(
        ...
        try:
            ...
            correlated_df = stream_a_prefixed.join(
                stream_b_prefixed, final_condition, correlation_type
            )

            # Add correlation metadata
            result_df = (
                correlated_df.withColumn("correlation_timestamp", current_timestamp())
                .withColumn("correlation_window_used", lit(correlation_window))
                .withColumn("correlation_keys_used", lit(",".join(correlation_keys)))
                .withColumn(
                    "time_difference_seconds",
                    when(
                        col(f"a_{time_column_a}").isNotNull()
                        & col(f"b_{time_column_b}").isNotNull(),
                        spark_abs(
                            col(f"a_{time_column_a}").cast("long")
                            - col(f"b_{time_column_b}").cast("long")
                        ),
                    ).otherwise(lit(None)),
                )
            )
            ...
            self.logger.info(
                f"Created cross-stream correlation on keys {correlation_keys} with {correlation_window} window"
            )
            return result_df
        ...
        try:
            ...
            behavior_sessionized = (
                ...
                .withColumn(
                    "session_boundary",
                    when(
                        col("time_gap_minutes")
                        > expr(f"interval '{session_gap}'").cast("long") / 60,
                        lit(1),
                    ).otherwise(lit(0)),
                )
                .withColumn(
            ...
            transaction_sessionized = transaction_df.withWatermark(
                "timestamp", "5 minutes"
            ).select(
                col("user_id").alias("tx_user_id"),
                col("timestamp").alias("tx_timestamp"),
            ...
            session_joined = behavior_sessionized.join(
                ...
                & (
                    col("tx_timestamp")
                    <= col("behavior_timestamp") + expr(f"interval '{session_gap}'")
                ),
                "left_outer",
            )
            ...
            session_analyzed = (
                ...
                .withColumn(
                    ...
                    when(
                        col("behavior_product_id") == col("tx_product_id"),
                        lit("Exact Match"),
                    )
                    .when(
                        col("behavior_product_id").isNotNull()
                        & col("tx_product_id").isNotNull(),
                        lit("Different Product"),
                    )
                    .when(col("tx_product_id").isNotNull(), lit("No Behavior Product"))
                    .otherwise(lit("No Purchase")),
                )
                .withColumn("session_join_timestamp", current_timestamp())
            )
