...
class StreamDeduplicator:
    ...
    def deduplicate_exact_events(
        ...
        try:
            ...
            df_with_key = watermarked_df.withColumn(
                "dedup_composite_key", composite_key_expr
            )

            # Add row number based on strategy
            if keep_strategy == "first":
                window_spec = Window.partitionBy("dedup_composite_key").orderBy(
                    col(watermark_column).asc()
                )
            elif keep_strategy in ("last", "latest_timestamp"):
                window_spec = Window.partitionBy("dedup_composite_key").orderBy(
            ...
            df_with_rank = df_with_key.withColumn(
                "dedup_rank", row_number().over(window_spec)
            )
            ...
            result_df = (
                ...
                .withColumn("deduplication_method", lit("exact"))
                .withColumn("deduplication_keys", lit(",".join(dedup_keys)))
                .withColumn("deduplication_strategy", lit(keep_strategy))
                .drop("dedup_composite_key", "dedup_rank")
            )

            self.logger.info(
                f"Applied exact deduplication on keys: {dedup_keys} with strategy: {keep_strategy}"
            )
            return result_df
    ...
    def deduplicate_transactions(
        ...
        try:
            ...
            exact_dedup_df = business_key_df.withColumn(
                "exact_rank", row_number().over(exact_dedup_window)
            ).filter(col("exact_rank") == 1)

            # Fuzzy deduplication for similar transactions
            # Group by user and time window for fuzzy matching
            fuzzy_window_spec = Window.partitionBy(
                "user_id", window(col("timestamp"), "10 minutes")
            ).orderBy("timestamp")

            fuzzy_analysis_df = (
                exact_dedup_df.withColumn(
                    "prev_amount", lag("total_amount").over(fuzzy_window_spec)
                )
                .withColumn("prev_product", lag("product_id").over(fuzzy_window_spec))
                .withColumn(
                    "prev_payment", lag("payment_method").over(fuzzy_window_spec)
                )
                .withColumn("prev_timestamp", lag("timestamp").over(fuzzy_window_spec))
            )

            # Calculate similarity score
            similarity_df = (
                fuzzy_analysis_df.withColumn(
                    "amount_similarity",
                    when(
                        col("prev_amount").isNotNull(),
                        1.0
                        - spark_abs(col("total_amount") - col("prev_amount"))
                        / greatest(col("total_amount"), col("prev_amount")),
                    ).otherwise(lit(0.0)),
                )
                .withColumn(
                    "product_similarity",
                    when(col("product_id") == col("prev_product"), lit(1.0)).otherwise(
                        lit(0.0)
                    ),
                )
                .withColumn(
                    "payment_similarity",
                    when(
                        col("payment_method") == col("prev_payment"), lit(1.0)
                    ).otherwise(lit(0.0)),
                )
                .withColumn(
                    "time_diff_seconds",
                ...
                .withColumn(
                    ...
                    when(
                        ...
                        & (col("time_diff_seconds") <= 300),  # 5 minutes
            ...
            fuzzy_flagged_df = similarity_scored_df.withColumn(
                "is_fuzzy_duplicate",
                when(
                    col("overall_similarity") >= similarity_threshold, lit(True)
                ).otherwise(lit(False)),
            ).withColumn(
                "duplicate_type",
                when(col("exact_rank") > 1, lit("exact"))
                .when(col("is_fuzzy_duplicate"), lit("fuzzy"))
                .otherwise(lit("unique")),
            )

            # Keep only non-fuzzy duplicates
            final_dedup_df = fuzzy_flagged_df.filter(~col("is_fuzzy_duplicate"))

            # Add deduplication metadata
            result_df = (
                final_dedup_df.withColumn(
                    "deduplication_timestamp", current_timestamp()
                )
                .withColumn("deduplication_method", lit("transaction_business_logic"))
                .withColumn("similarity_threshold_used", lit(similarity_threshold))
                .select(
                    ...
                    col("duplicate_type"),
                    col("overall_similarity"),
                    col("deduplication_timestamp"),
                    col("deduplication_method"),
                    col("similarity_threshold_used"),
                )
            )

            self.logger.info(
                f"Applied transaction deduplication with {similarity_threshold} similarity threshold"
            )
            return result_df
    ...
    def deduplicate_user_behavior(
        ...
        try:
            ...
            if session_dedup:
                ...
                session_window = Window.partitionBy(session_dedup_keys).orderBy(
                    "timestamp"
                )

                session_dedup_df = (
                    watermarked_df.withColumn(
                        "session_event_rank", row_number().over(session_window)
                    )
                    .withColumn(
                        "prev_event_timestamp", lag("timestamp").over(session_window)
                    )
                    .withColumn(
                        ...
                        when(
                            ...
                            col("timestamp").cast("long")
                            - col("prev_event_timestamp").cast("long"),
                        ).otherwise(lit(None)),
                    )
                )
                ...
                time_threshold = (
                    int(event_dedup_window.split()[0])
                    if event_dedup_window.endswith("seconds")
                    else 30
                )
                ...
                filtered_df = watermarked_df.withColumn(
                    "temporal_rank", row_number().over(temporal_window)
            ...
            rapid_fire_df = (
                filtered_df.withColumn(
                    "prev_user_timestamp", lag("timestamp").over(user_window)
                )
                .withColumn(
                    "time_since_prev_user_event",
                    when(
                        col("prev_user_timestamp").isNotNull(),
                        col("timestamp").cast("long")
                        - col("prev_user_timestamp").cast("long"),
                    ).otherwise(lit(None)),
                )
                .withColumn(
                    "is_rapid_fire",
                    when(
                        col("time_since_prev_user_event") <= 1, lit(True)
                    ).otherwise(  # 1 second threshold
                        lit(False)
                    ),
                )
            )
            ...
            product_window = Window.partitionBy(
                "product_view_key", window(col("timestamp"), "5 minutes")
            ).orderBy("timestamp")

            product_dedup_df = product_view_df.withColumn(
                "product_view_rank",
                when(
                    col("product_view_key").isNotNull(),
            ...
            # Add deduplication metadata
            result_df = (
                product_dedup_df.withColumn(
                    "deduplication_timestamp", current_timestamp()
                )
                .withColumn(
                    "deduplication_method",
                    when(session_dedup, lit("session_and_temporal")).otherwise(
                        lit("temporal_only")
                    ),
                )
                .withColumn("rapid_fire_filtered", ~col("is_rapid_fire"))
                .select(
            ...
            self.logger.info(
                f"Applied user behavior deduplication with session_dedup={session_dedup}"
            )
            return result_df
    ...
    def create_deduplication_statistics(
        ...
        try:
            ...
            original_counts = (
                original_df.withWatermark("timestamp", "2 minutes")
            ...
            dedup_counts = (
                deduplicated_df.withWatermark("timestamp", "2 minutes")
                .groupBy(window(col("timestamp"), window_duration))
                .agg(
            ...
            stats_df = original_counts.join(
                ...
                coalesce(original_counts.window, dedup_counts.window).alias(
                    "time_window"
                ),
                coalesce(col("original_count"), lit(0)).alias("original_count"),
                coalesce(col("deduplicated_count"), lit(0)).alias("deduplicated_count"),
                coalesce(col("original_unique_keys"), lit(0)).alias(
                    "original_unique_keys"
                ),
                coalesce(col("deduplicated_unique_keys"), lit(0)).alias(
                    "deduplicated_unique_keys"
                ),
            )
            ...
            metrics_df = (
                ...
                .withColumn(
                    ...
                    when(
                        col("original_count") > 0,
                        (col("deduplicated_unique_keys") * 100.0)
                        / col("original_count"),
                    ).otherwise(lit(100.0)),
                )
                .withColumn("statistics_timestamp", current_timestamp())
                .withColumn("dedup_keys_analyzed", lit(",".join(dedup_keys)))
            )
    ...
    def handle_late_arrivals(
        ...
        try:
            ...
            delay_df = (
                watermarked_df.withColumn("processing_timestamp", current_timestamp())
                .withColumn(
                    "processing_delay_seconds",
                    col("processing_timestamp").cast("long")
                    - col(watermark_column).cast("long"),
                )
                .withColumn(
                    "is_late_arrival",
                    col("processing_delay_seconds")
                    > expr(f"interval '{grace_period}'").cast("long"),
                )
            )
            ...
            if late_data_strategy == "flag":
                ...
                result_df = delay_df.withColumn(
                    ...
                    "data_quality_flag",
                    when(col("is_late_arrival"), lit("late_arrival")).otherwise(
                        lit("on_time")
                    ),
                )
                ...
                # Drop late arrivals
                result_df = delay_df.filter(~col("is_late_arrival")).withColumn(
                    "late_arrival_handling", lit("dropped_late_data")
                )
                ...
                result_df = delay_df.withColumn(
                    "processing_stream",
                    when(col("is_late_arrival"), lit("late_data_stream")).otherwise(
                        lit("main_stream")
                    ),
                ).withColumn("requires_reprocessing", col("is_late_arrival"))
            else:
            ...
            final_df = (
                ...
                .withColumn("grace_period_used", lit(grace_period))
                .withColumn("late_arrival_processing_timestamp", current_timestamp())
            )

            self.logger.info(
                f"Applied late arrival handling with strategy: {late_data_strategy}"
            )
            return final_df
