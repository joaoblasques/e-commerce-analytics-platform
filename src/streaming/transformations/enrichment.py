...
from pyspark.sql.functions import (
    col, lit, when, coalesce, regexp_extract, split,
    current_timestamp, date_format, hour, dayofweek,
    upper, lower, trim, size, array_contains,
    udf, broadcast
)
...
class DataEnrichmentPipeline:
    ...
    def _enrich_customer_data(self, df: DataFrame) -> DataFrame:
        ...
        enriched_df = df.withColumn(
            ...
            when(col("customer_tier").isNull(), lit("Unknown"))
            .otherwise(upper(trim(col("customer_tier"))))
        )
    ...
    def _enrich_geographic_data(self, df: DataFrame) -> DataFrame:
        ...
        enriched_df = df.withColumn(
            ...
            .when(col("location").rlike("(?i)california|ca|san.francisco|los.angeles"), lit("West"))
    ...
    def _enrich_business_metrics(self, df: DataFrame) -> DataFrame:
        ...
        enriched_df = df.withColumn(
            ...
            when(col("total_amount").isNotNull(),
                 col("total_amount") * lit(0.2))  # 20% margin assumption
            .otherwise(lit(0.0))
        )
        ...
        enriched_df = enriched_df.withColumn(
            "bulk_purchase",
            when(col("quantity") >= 5, lit(True)).otherwise(lit(False))
        )
    ...
    def _enrich_risk_indicators(self, df: DataFrame) -> DataFrame:
        ...
        enriched_df = df.withColumn(
            ...
            .when(col("is_weekend") & (col("hour_of_day") < 6), lit(2))  # Late weekend = medium risk
