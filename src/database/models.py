...
class Customer(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("account_status IN ('active', 'inactive', 'suspended')", name="check_account_status"),
...
class Order(Base):
    ...
    # Customer reference
    user_id: Mapped[str] = mapped_column(String(50), ForeignKey("customers.user_id"), nullable=False)
    ...
    subtotal: Mapped[Decimal] = mapped_column(DECIMAL(12, 2), nullable=False, default=0.00)
    tax_amount: Mapped[Decimal] = mapped_column(DECIMAL(12, 2), nullable=False, default=0.00)
    shipping_cost: Mapped[Decimal] = mapped_column(DECIMAL(12, 2), nullable=False, default=0.00)
    discount_amount: Mapped[Decimal] = mapped_column(DECIMAL(12, 2), nullable=False, default=0.00)
    total_amount: Mapped[Decimal] = mapped_column(DECIMAL(12, 2), nullable=False, default=0.00)
    ...
    __table_args__ = (
        ...
        CheckConstraint("order_status IN ('pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded')", name="check_order_status"),
        CheckConstraint("payment_status IN ('pending', 'paid', 'failed', 'refunded', 'cancelled')", name="check_payment_status"),
...
class OrderItem(Base):
    ...
    # Primary key
    order_item_id: Mapped[str] = mapped_column(String(50), primary_key=True, default=lambda: str(uuid.uuid4()))
...
class Category(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text)
    parent_category_id: Mapped[Optional[str]] = mapped_column(String(50), ForeignKey("categories.category_id"))
    ...
    parent: Mapped[Optional["Category"]] = relationship("Category", remote_side="Category.category_id")
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('active', 'inactive')", name="check_category_status"),
...
class Supplier(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('active', 'inactive', 'suspended')", name="check_supplier_status"),
    )