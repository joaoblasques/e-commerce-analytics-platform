...
class SeedDataGenerator:
        ...
        for i in range(count):
            ...
            customer = Customer(
                ...
                last_login=self.faker.date_time_between(
                    start_date=registration_date, end_date="now"
                )
                if random.random() > 0.1
                else None,
                account_status=random.choices(
                    ["active", "inactive", "suspended"], weights=[0.85, 0.12, 0.03]
                )[0],
    ...
    def generate_products(
        self, session: Session, suppliers: List[Supplier], count: int = 500
    ) -> List[Product]:
        ...
        for i in range(count):
            ...
            product = Product(
                ...
                status=random.choices(
                    ["active", "inactive", "discontinued"], weights=[0.8, 0.15, 0.05]
                )[0],
    ...
    def generate_orders(
        ...
        for i in range(count):
            ...
            order = Order(
                ...
                delivered_date=order_date + timedelta(days=random.randint(3, 10))
    ...
    def generate_order_items(
        self, session: Session, orders: List[Order], products: List[Product]
    ) -> List[OrderItem]:
        ...
        for order in orders:
            ...
            order.shipping_cost = (
                Decimal("9.99") if order_subtotal < 50 else Decimal("0.00")
            )
            order.discount_amount = order_subtotal * Decimal(
