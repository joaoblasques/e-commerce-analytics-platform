...
class StructuredLogger:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        # File handler
        if (
            self.config.output_destination in ["file", "both"]
            and self.config.log_file_path
        ):
    ...
    def log_request(
        ...
        getattr(self, level)(
            f"{method} {path} {status_code} {duration_ms:.2f}ms", extra=extra
        )
    ...
    def log_database_query(
        ...
        **kwargs,
        ...
        getattr(self, level)(
            f"Database query completed in {duration_ms:.2f}ms", extra=extra
        )
    ...
    def log_kafka_event(
        ...
        self.info(f"Kafka {event_type}: {topic}[{partition}]@{offset}", extra=extra)
...
def configure_from_settings(settings: Dict[str, Any]) -> LogConfig:
    ...
    return LogConfig(
        ...
        elasticsearch_index=logging_settings.get("elasticsearch", {}).get(
            "index", "ecap-logs"
        ),
    )
