...
class MerchantRiskScorer:
    ...
    def calculate_merchant_risk_scores(
        self, transactions_df: DataFrame, lookback_days: int = 30
    ) -> DataFrame:
        """
    ...
    def _calculate_risk_indicators(
        self, transactions_df: DataFrame, merchant_metrics: DataFrame
    ) -> DataFrame:
        ...
        risk_df = enriched_df.groupBy("merchant_id").agg(
            ...
            F.sum(
                F.when(
                    F.col("price") > F.col("avg_transaction_amount") * 3, 1
                ).otherwise(0)
            ).alias("high_amount_transactions"),
            F.sum(F.when(F.hour("timestamp").between(0, 5), 1).otherwise(0)).alias(
                "unusual_hour_transactions"
            ),
            F.sum(F.when(F.dayofweek("timestamp").isin([1, 7]), 1).otherwise(0)).alias(
                "weekend_transactions"
            ),
            # Round amounts (potential fraud indicator)
            F.sum(
                F.when(
                    (F.col("price") % 100 == 0) & (F.col("price") > 100), 1
                ).otherwise(0)
            ).alias("round_amount_transactions"),
        )
        ...
        risk_df = (
            ...
            .withColumn(
                ...
                / F.greatest(F.col("merchant_age_days"), F.lit(1)),
            )
            .withColumn(
                "revenue_per_customer",
                F.col("total_revenue")
                / F.greatest(F.col("unique_customers"), F.lit(1)),
            )
            .withColumn(
                "amount_volatility",
                F.when(
    ...
    def _calculate_final_risk_scores(self, risk_df: DataFrame) -> DataFrame:
        ...
        score_df = score_df.withColumn(
            ...
            .when(
                F.col("transaction_count") < 50, 0.6
            )  # Low volume is moderately risky
            .otherwise(
                1.0 - F.percent_rank().over(volume_percentile_window)
            ),  # Higher volume = lower risk
        )

        # Customer diversity risk (low diversity = higher risk)
        customers_per_transaction = F.col("unique_customers") / F.col(
            "transaction_count"
        )
        score_df = score_df.withColumn(
        ...
        score_df = score_df.withColumn(
            "volatility_risk_score", F.percent_rank().over(volatility_window)
        )
        ...
        score_df = score_df.withColumn(
            ...
            (
                ...
                + F.col("customer_diversity_score")
                * weights["customer_diversity_weight"]
                + F.col("volatility_risk_score") * weights["amount_volatility_weight"]
                + F.col("behavioral_risk_score")
                * (weights["chargeback_rate_weight"] + weights["refund_rate_weight"])
        ...
        score_df = score_df.withColumn(
            "risk_flags",
            F.array(
                F.when(F.col("transaction_count") < 10, "LOW_VOLUME").otherwise(
                    F.lit(None)
                ),
                F.when(
                    F.col("customer_diversity_score") > 0.7, "LOW_CUSTOMER_DIVERSITY"
                ).otherwise(F.lit(None)),
                F.when(
                    F.col("volatility_risk_score") > 0.8, "HIGH_AMOUNT_VOLATILITY"
                ).otherwise(F.lit(None)),
                F.when(
                    F.col("unusual_hour_rate") > 0.3, "HIGH_UNUSUAL_HOURS"
                ).otherwise(F.lit(None)),
                F.when(
                    F.col("round_amount_rate") > 0.5, "HIGH_ROUND_AMOUNTS"
                ).otherwise(F.lit(None)),
                F.when(F.col("merchant_age_days") < 30, "NEW_MERCHANT").otherwise(
                    F.lit(None)
                ),
            ).cast("array<string>"),
        )
        ...
        score_df = score_df.withColumn(
            "risk_flags", F.expr("filter(risk_flags, x -> x is not null)")
        )

        return score_df

    def get_high_risk_merchants(
        self, merchant_scores_df: DataFrame, min_risk_score: float = 0.6
    ) -> DataFrame:
        """
    ...
    def analyze_merchant_transaction_patterns(
        self, transactions_df: DataFrame, merchant_id: str
    ) -> Dict:
        """
        ...
        hourly_pattern = (
            merchant_data.groupBy(F.hour("timestamp").alias("hour"))
        ...
        daily_pattern = (
            merchant_data.groupBy(F.date_format("timestamp", "EEEE").alias("day"))
            .agg(F.count("*").alias("count"))
            .collect()
        )
        ...
        return {
            ...
            "basic_stats": {
                ...
                "amount_stddev": float(stats["amount_stddev"])
                if stats["amount_stddev"]
                else 0.0,
            ...
            "amount_distribution": {
                row["amount_range"]: row["count"] for row in amount_distribution
            },
        }
    ...
    def save_risk_profiles(
        self, merchant_scores_df: DataFrame, output_path: str
    ) -> None:
        """
