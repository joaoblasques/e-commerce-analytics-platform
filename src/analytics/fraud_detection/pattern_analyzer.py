...
class TransactionPatternAnalyzer:
    ...
    def analyze_velocity_patterns(
        self, df: DataFrame, time_windows: List[str] = ["1 hour", "1 day", "7 days"]
    ) -> DataFrame:
        """
        ...
        for window in time_windows:
            ...
            result_df = result_df.withColumn(
                f"transaction_count_{window_suffix}", F.count("*").over(window_spec)
            )

            # Sum amount in window
            result_df = result_df.withColumn(
                f"total_amount_{window_suffix}", F.sum("price").over(window_spec)
            )
    ...
    def analyze_amount_patterns(self, df: DataFrame) -> DataFrame:
        ...
        result_df = (
            df.withColumn("customer_avg_amount", F.avg("price").over(customer_window))
        ...
        result_df = result_df.withColumn(
            "is_round_amount", (F.col("price") % 100 == 0) & (F.col("price") > 0)
        ).withColumn(
            "amount_ends_with_99",
        ...
        result_df = result_df.withColumn(
            "amount_percentile", F.percent_rank().over(percentile_window)
        )
    ...
    def analyze_time_patterns(self, df: DataFrame) -> DataFrame:
        ...
        result_df = result_df.withColumn(
            "customer_common_hours", F.collect_list("hour").over(customer_time_window)
        ).withColumn("customer_avg_hour", F.avg("hour").over(customer_time_window))
        ...
        customer_time_ordered_window = Window.partitionBy("customer_id").orderBy(
            "timestamp"
        )
        ...
        result_df = result_df.withColumn(
            ...
            F.when(
                ...
                - F.unix_timestamp("prev_transaction_time"),
            ).otherwise(F.lit(None)),
        )

        # Flag rapid succession (< 60 seconds)
        result_df = result_df.withColumn(
            "is_rapid_succession", F.col("seconds_since_last_transaction") < 60
        )
    ...
    def analyze_geographic_patterns(self, df: DataFrame) -> DataFrame:
        ...
        location_time_window = (
            Window.partitionBy("customer_id", "location")
            .orderBy("timestamp")
            .rangeBetween(-3600, 0)
        )  # 1 hour window

        result_df = result_df.withColumn(
            "location_transaction_count_1h", F.count("*").over(location_time_window)
        )

        # Multiple location usage pattern
        multi_location_window = (
            Window.partitionBy("customer_id")
            .orderBy("timestamp")
            .rangeBetween(-3600, 0)
        )  # 1 hour window
    ...
    def analyze_merchant_patterns(self, df: DataFrame) -> DataFrame:
        ...
        result_df = (
            ...
            .withColumn(
                "merchant_total_amount", F.sum("price").over(merchant_customer_window)
            )
            .withColumn(
                "merchant_avg_amount", F.avg("price").over(merchant_customer_window)
            )
        )
    ...
    def analyze_payment_patterns(self, df: DataFrame) -> DataFrame:
        ...
        result_df = result_df.withColumn(
            ...
            ~F.array_contains(
                F.col("customer_payment_methods"), F.col("payment_method")
            ),
        )
