...
class FraudAlertPrioritizer:
    ...
    def prioritize_alerts(self, alerts_df: DataFrame,
                         customer_risk_df: Optional[DataFrame] = None,
                         merchant_risk_df: Optional[DataFrame] = None,
                         historical_performance_df: Optional[DataFrame] = None) -> DataFrame:
        """
        ...
        if historical_performance_df:
            result_df = self._add_historical_accuracy_factor(result_df, historical_performance_df)
        else:
    ...
    def _add_customer_risk_factor(self, df: DataFrame, customer_risk_df: DataFrame) -> DataFrame:
        """Add customer risk factor from customer risk scores."""
        return df.join(
            customer_risk_df.select("customer_id", 
                                  F.col("customer_risk_score").alias("customer_risk_factor")),
            "customer_id",
            "left"
        ).fillna({"customer_risk_factor": 0.5})
    
    def _add_merchant_risk_factor(self, df: DataFrame, merchant_risk_df: DataFrame) -> DataFrame:
        """Add merchant risk factor from merchant risk scores."""
        return df.join(
            merchant_risk_df.select("merchant_id",
    ...
    def _calculate_pattern_complexity_factor(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            ...
            F.when(F.array_contains("fraud_alerts.severity", "CRITICAL"),
                  F.col("pattern_complexity_factor") * 1.5
            ).otherwise(F.col("pattern_complexity_factor"))
        )
    
    def _add_historical_accuracy_factor(self, df: DataFrame, historical_df: DataFrame) -> DataFrame:
        ...
        rule_accuracy = historical_df.groupBy("rule_id").agg(
            F.avg(F.when(F.col("resolution") == "TRUE_POSITIVE", 1.0).otherwise(0.0)).alias("accuracy")
        )
        ...
        ).select("alert_data.*", "historical_accuracy_factor").fillna({"historical_accuracy_factor": 0.5})
    ...
    def _calculate_business_criticality_factor(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            ...
            F.when(F.col("customer_risk_factor") > 0.7, F.col("business_criticality_factor") * 1.2)
            .when(F.col("merchant_risk_factor") > 0.7, F.col("business_criticality_factor") * 1.1)
    ...
    def _assign_priority_levels(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            "alert_priority",
            F.when(F.col("priority_score") >= thresholds[AlertPriority.CRITICAL], "CRITICAL")
    ...
    def _add_recommended_actions(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            "recommended_actions",
            F.when(F.col("alert_priority") == "CRITICAL",
                  F.array(F.lit("IMMEDIATE_REVIEW"), F.lit("BLOCK_CUSTOMER"), F.lit("ESCALATE_TO_SENIOR"))
            ).when(F.col("alert_priority") == "HIGH",
                  F.array(F.lit("REVIEW_WITHIN_1_HOUR"), F.lit("VERIFY_CUSTOMER_CONTACT"), F.lit("CHECK_RECENT_ACTIVITY"))
            ).when(F.col("alert_priority") == "MEDIUM",
    ...
    def _estimate_investigation_time(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            ...
            F.col("estimated_investigation_time_minutes") + (F.size("triggered_rules") * 5)
        )
    
    def get_priority_queue(self, prioritized_alerts_df: DataFrame, 
                          max_alerts: int = 100,
                          priority_filter: Optional[List[str]] = None) -> DataFrame:
        """
        ...
        result_df = result_df.withColumn("queue_position", F.row_number().over(window_spec))
    ...
    def get_alert_summary_metrics(self, prioritized_alerts_df: DataFrame) -> Dict:
        ...
        """
        # Count by priority
        priority_counts = prioritized_alerts_df.groupBy("alert_priority").agg(
            F.count("*").alias("count"),
        ...
        overall_stats = prioritized_alerts_df.agg(
            ...
            F.avg("priority_score").alias("avg_priority_score"),
            F.sum("estimated_investigation_time_minutes").alias("total_investigation_time")
        ).collect()[0]
        
        # Top rules triggered
        top_rules = prioritized_alerts_df.select(F.explode("triggered_rules").alias("rule"))\
                                       .groupBy("rule")\
        ...
        return {
            ...
            "overall_stats": {
                ...
                "total_financial_exposure": float(overall_stats["total_financial_exposure"]),
                "avg_priority_score": float(overall_stats["avg_priority_score"]),
                "total_investigation_time_hours": overall_stats["total_investigation_time"] / 60
            },
            "top_triggered_rules": [
    ...
    def update_priority_thresholds(self, new_thresholds: Dict[AlertPriority, float]) -> None:
        """
    ...
    def save_prioritized_alerts(self, prioritized_alerts_df: DataFrame, output_path: str) -> None:
        """
