...
class AnomalyDetector:
    ...
    def __init__(
        self, spark: SparkSession, stream_source_path: str = "data/delta/transactions"
    ):
        self.spark = spark
    ...
    def detect_anomalies(self, df: DataFrame, feature_cols: list) -> DataFrame:
        ...
        # In real-time streaming, we use statistical methods for anomaly detection
        # instead of batch-oriented algorithms like LOF
        # For streaming scenarios, statistical approaches are more appropriate and performant
        ...
        joined_df = df.join(
            aggregated_df,
            F.col("timestamp").between(F.col("window.start"), F.col("window.end")),
        )
        ...
        anomalies_df = joined_df.withColumn(
            ...
            F.when(
                (
                    F.abs(F.col("price") - F.col("mean_price"))
                    > 3 * F.col("stddev_price")
                )
    ...
    def run_detection(self, feature_cols: list, output_path: str = None):
        ...
        if output_path:
            query = query.format("delta").option(
