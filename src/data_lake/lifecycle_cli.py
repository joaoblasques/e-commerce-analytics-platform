...
from .lifecycle_manager import (
    DataLifecycleManager, RetentionRule, ArchivePolicy, 
    LineageRecord, RetentionPolicy, DataTier
)
...
class LifecycleCLI:
    ...
    def apply_retention(self, args):
        ...
        try:
            ...
            tables_to_process = args.tables if args.tables else list(self.lifecycle_manager.retention_rules.keys())
            ...
            for table_name in tables_to_process:
                ...
                try:
                    ...
                    # Print results
                    print(f"   Hot records (0-{rule.hot_days} days): {result.get('hot_records', 0):,}")
                    print(f"   Warm records ({rule.hot_days}-{rule.warm_days} days): {result.get('warm_records', 0):,}")
                    print(f"   Cold records ({rule.warm_days}-{rule.cold_days} days): {result.get('cold_records', 0):,}")
        ...
        try:
            ...
            if operations:
                ...
                for i, op in enumerate(operations[-10:]):  # Show last 10 operations
                    ...
                    print(f"   {i+1:2d}. {op['operation']} at {timestamp} ({op['record_count']:,} records)")
    ...
    def optimize_storage(self, args):
        ...
        try:
            ...
            # Display metrics
            total_size_gb = sum(m.size_bytes for m in metrics.values()) / (1024 * 1024 * 1024)
            ...
            print(f"\n📋 Per-table metrics:")
            print(f"{'Table':<25} {'Size (GB)':<12} {'Tier':<15} {'Cost ($)':<10} {'Recommendation'}")
            print("-" * 80)
            ...
            for table_name, table_metrics in metrics.items():
                ...
                if days_since_access > 90 and current_tier in ['standard', 'infrequent']:
                ...
                print(f"{table_name:<25} {size_gb:<12.2f} {current_tier:<15} {cost:<10.2f} {recommendation}")
            
            # Generate optimization recommendations
            recommendations = self.lifecycle_manager._generate_recommendations(metrics, {})
    ...
    def generate_report(self, args):
        ...
        try:
            ...
            if retention_summary:
                ...
                total_hot = sum(r.get('hot_records', 0) for r in retention_summary.values() if isinstance(r, dict))
                total_warm = sum(r.get('warm_records', 0) for r in retention_summary.values() if isinstance(r, dict))
                total_cold = sum(r.get('cold_records', 0) for r in retention_summary.values() if isinstance(r, dict))
            ...
            print(f"   Monthly cost: ${storage_summary.get('total_monthly_cost_usd', 0):.2f}")
            ...
            if by_tier:
                ...
                for tier, tier_data in by_tier.items():
                    print(f"   {tier.capitalize()}: {tier_data['table_count']} tables, "
                          f"{tier_data['total_size_gb']:.2f} GB, ${tier_data['total_cost_usd']:.2f}")
            
            # Lineage summary
            lineage_summary = report['lineage_summary']
            print(f"\n🔗 Lineage Summary:")
            print(f"   Total operations tracked: {lineage_summary.get('total_operations', 0)}")
            print(f"   Tables with lineage: {lineage_summary.get('tables_tracked', 0)}")
            print(f"   Recent operations (7 days): {lineage_summary.get('recent_operations', 0)}")
            ...
            # Save report
            output_file = args.output or f"lifecycle_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def config_table(self, args):
        ...
        try:
            ...
            if any([args.hot_days, args.warm_days, args.cold_days, args.archive_days, args.delete_days]):
                retention_rule = RetentionRule(
                    ...
                    archive_after_days=args.archive_days or existing_rule.archive_after_days,
                    delete_after_days=args.delete_days or existing_rule.delete_after_days,
                    partition_column=args.partition_column or existing_rule.partition_column,
                    enabled=args.enabled if args.enabled is not None else existing_rule.enabled
                )
            ...
            if any([args.archive_path, args.archive_format, args.compression]):
                archive_policy = ArchivePolicy(
                    ...
                    enabled=args.enabled if args.enabled is not None else existing_policy.enabled
                )
    ...
    def list_tables(self, args):
        ...
        try:
            ...
            for table_name in sorted(all_tables):
                ...
                retention_status = "Enabled" if retention_rule and retention_rule.enabled else "Disabled"
                archive_status = "Enabled" if archive_policy and archive_policy.enabled else "Disabled"
                hot_days = retention_rule.hot_days if retention_rule else "N/A"
                delete_days = retention_rule.delete_after_days if retention_rule else "Never"
                
                print(f"{table_name:<25} {retention_status:<12} {archive_status:<12} {hot_days:<10} {delete_days}")
            
            # Show summary for each table if detailed view requested
            if args.detailed:
                for table_name in sorted(all_tables):
                    print(f"\n📊 {table_name}:")
                    summary = self.config_manager.get_table_lifecycle_summary(table_name)
                    
                    print(f"   Retention: {summary['retention']['hot_days']}/{summary['retention']['warm_days']}/{summary['retention']['cold_days']} days")
                    if summary['retention']['archive_after_days']:
                        print(f"   Archive after: {summary['retention']['archive_after_days']} days")
                    if summary['retention']['delete_after_days']:
                        print(f"   Delete after: {summary['retention']['delete_after_days']} days")
                    
                    print(f"   Archive: {summary['archiving']['format']} format, {summary['archiving']['compression']} compression")
...
Examples:
  ...
  python -m src.data_lake.lifecycle_cli list --detailed
    ...
    # Global arguments
    parser.add_argument("--base-path", default="s3a://data-lake/delta",
    ...
    retention_parser.add_argument("--tables", nargs="+", help="Specific tables to process")
    retention_parser.add_argument("--dry-run", action="store_true", help="Show what would be done without executing")
    ...
    lineage_parser.add_argument("--table", required=True, help="Table to trace lineage for")
    lineage_parser.add_argument("--depth", type=int, default=3, help="Maximum lineage depth")
    ...
    optimize_parser.add_argument("--strategy", choices=["cost_optimized", "performance_optimized", "compliance_optimized"],
                                default="cost_optimized", help="Optimization strategy")
    optimize_parser.add_argument("--output", help="Output file for optimization analysis")
    ...
    # Config command
    config_parser = subparsers.add_parser("config", help="Configure table lifecycle policies")
    ...
    config_parser.add_argument("--partition-column", help="Partition column for time-based retention")
    config_parser.add_argument("--archive-path", help="Archive storage path")
    config_parser.add_argument("--archive-format", choices=["parquet", "orc", "delta"], help="Archive format")
    config_parser.add_argument("--compression", choices=["gzip", "snappy", "lz4", "zstd"], help="Compression type")
    config_parser.add_argument("--partition-by", nargs="+", help="Archive partition columns")
    ...
    list_parser.add_argument("--detailed", action="store_true", help="Show detailed configuration")
