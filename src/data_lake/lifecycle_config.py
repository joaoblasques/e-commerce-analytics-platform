...
from .lifecycle_manager import RetentionRule, ArchivePolicy, DataTier
...
class LifecycleConfigManager:
    ...
    def get_tier_configuration(self, optimization_strategy: str = "cost_optimized") -> Dict:
        """Get storage tier configuration for a specific strategy."""
        return self.tier_configurations.get(optimization_strategy, 
                                           self.tier_configurations["cost_optimized"])
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for name, rule in self.retention_rules.items():
            ...
            if rule.archive_after_days and rule.archive_after_days <= rule.cold_days:
                issues.append(f"Table {name}: archive_after_days should be greater than cold_days")
            
            if (rule.delete_after_days and rule.archive_after_days and 
                rule.delete_after_days <= rule.archive_after_days):
                issues.append(f"Table {name}: delete_after_days should be greater than archive_after_days")
        
        # Validate archive policies
        for name, policy in self.archive_policies.items():
            if not policy.archive_path.startswith(('s3a://', 's3://', '/', 'hdfs://')):
                issues.append(f"Table {name}: archive_path should be a valid storage path")
            ...
            if policy.compression not in ['gzip', 'snappy', 'lz4', 'zstd', None]:
                issues.append(f"Table {name}: unsupported compression {policy.compression}")
    ...
    def _estimate_data_lifecycle(self, rule: RetentionRule) -> Dict[str, str]:
        ...
        if rule.archive_after_days:
            lifecycle["archive_period"] = f"{rule.archive_after_days - rule.cold_days} days"
        
        if rule.delete_after_days:
            if rule.archive_after_days:
                lifecycle["archived_storage"] = f"{rule.delete_after_days - rule.archive_after_days} days"
