...
class DataLifecycleManager:
    ...
    def _initialize_metadata_tables(self):
        ...
        try:
            ...
            lineage_table_path = (
                f"{self.delta_manager.base_path}/{self.metadata_table}_lineage"
            )
            ...
            if not self.delta_manager.table_exists(f"{self.metadata_table}_lineage"):
                ...
                logger.info(
                    f"Created lineage tracking table: {self.metadata_table}_lineage"
                )
            ...
            metrics_schema = StructType(
                [
                    ...
                    StructField(
                        "storage_cost_usd", StringType(), True
                    ),  # Decimal as string
            ...
            metrics_table_path = (
                f"{self.delta_manager.base_path}/{self.metadata_table}_metrics"
            )
            ...
            if not self.delta_manager.table_exists(f"{self.metadata_table}_metrics"):
                ...
                logger.info(
                    f"Created storage metrics table: {self.metadata_table}_metrics"
                )
    ...
    def track_lineage(self, lineage_record: LineageRecord) -> None:
        ...
        try:
            ...
            self.delta_manager.write_to_delta(
                lineage_df, f"{self.metadata_table}_lineage", mode="append"
            )
            ...
            logger.info(
                f"Tracked lineage for operation: {lineage_record.operation} on {lineage_record.table_name}"
            )
    ...
    def _apply_table_retention_policy(
        self, table_name: str, rule: RetentionRule, dry_run: bool
    ) -> Dict[str, int]:
        ...
        if rule.delete_after_days:
            ...
            if delete_count > 0 and not dry_run:
                self._delete_old_data(
                    table_name, delete_boundary, rule.partition_column
                )
    ...
    def _archive_data(self, table_name: str, archive_df: DataFrame) -> None:
        ...
        try:
            ...
            logger.info(
                f"Archived {archive_df.count()} records from {table_name} to {archive_path}"
            )
    ...
    def _delete_old_data(
        self, table_name: str, delete_boundary: datetime, partition_column: str
    ) -> None:
        ...
        try:
            ...
            deleted_count = self.delta_manager.delete_from_table(
    ...
    def _determine_optimal_tier(
        self, table_name: str, last_accessed: datetime
    ) -> DataTier:
    ...
    def _apply_cost_optimization(
        self, table_name: str, metrics: StorageMetrics
    ) -> None:
        """Apply cost optimization recommendations."""
        ...
        if current_tier != optimal_tier:
            logger.info(
                f"Recommend moving {table_name} from {current_tier.value} to {optimal_tier.value}"
            )
    ...
    def get_lineage_graph(self, table_name: str, depth: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            lineage_df = self.delta_manager.read_delta_table(
                f"{self.metadata_table}_lineage"
            )

            # Filter for the specific table
            table_lineage = lineage_df.filter(
                (col("table_name") == table_name) | (col("target_table") == table_name)
            ).collect()
    ...
    def generate_lifecycle_report(self) -> Dict[str, Any]:
        ...
        report["recommendations"] = self._generate_recommendations(
            storage_metrics, retention_results
        )
        ...
        for tier in DataTier:
            ...
            if tier_metrics:
                tier_summary[tier.value] = {
                    ...
                    "total_size_gb": sum(m.size_bytes for m in tier_metrics)
                    / (1024 * 1024 * 1024),
    ...
    def _generate_recommendations(
        ...
        total_size_gb = sum(m.size_bytes for m in storage_metrics.values()) / (
            1024 * 1024 * 1024
        )
    ...
    def save_configuration(self, config_path: str) -> None:
        ...
        try:
            config = {
                "retention_rules": [
                    asdict(rule) for rule in self.retention_rules.values()
                ],
                "archive_policies": [
                    asdict(policy) for policy in self.archive_policies.values()
                ],
            }
